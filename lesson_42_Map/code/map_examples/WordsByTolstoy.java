...
public class WordsByTolstoy
{
    public static void main(String[] args)
    {
        ...
        words = words.replaceAll("[^a-zA-Zа-яА-ЯёЁ\\s]", "").toLowerCase();

        String[] wordsArray = words.split(" ");

        for (String word : wordsArray)
        {
            // метод getOrDefault() либо возвращает текущее количество, либо 0, если слово еще не встречалось
            wordsMap.put(word, wordsMap.getOrDefault(word, 0) + 1);
        }

        wordsMap.forEach((word, number) -> System.out.println(String.format("Word '%s' repeated %d times",word, number)));
    }
}
